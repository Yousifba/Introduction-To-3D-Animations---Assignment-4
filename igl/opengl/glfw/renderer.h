...
public:
	...
	void IK();
	void IKRotations(int beginIndex, float alpha, Eigen::Vector3f rotationVec);
	void IKUpdateTransformations(int beginIndex, float alpha, Eigen::Vector3f rotationVec);
	void PrintArmsTips();
	void PrintDestination();
	void TranslateCamera();
	void RotateCamera();
	void RotateArmLinks(int root_index);
	void RotateArmLinkByKey(int key);
	void TranslateArm();
	void ScaleArm(double scale);
	void UpdateParentsTranslations(int beginIndex);
	bool isArm();
	Eigen::Vector4f GetRootCoords();
