...
static void glfw_mouse_press(GLFWwindow* window, int button, int action, int modifier)
{
  ...
  if (action == GLFW_PRESS)
  {
	  ...
	  if (scn->found_obj && !scn->collision_0_1)
	  {
...
static void glfw_mouse_scroll(GLFWwindow* window, double x, double y)
{
	...
	if (rndr->GetScene()->found_obj)
	{
		...
		{
			rndr->GetScene()->scales.at(rndr->GetScene()->selected_data_index) *= (1 + y * 0.1);
			rndr->GetScene()->data().MyScale(Eigen::Vector3f(1 + y * 0.1, 1 + y * 0.1, 1 + y * 0.1));
		}
	}
	else
	{
...
static void glfw_key_callback(GLFWwindow* window, int key, int scancode, int action, int modifier)
{
	...
	if (key == GLFW_KEY_ESCAPE && action == GLFW_PRESS)
		...
		case GLFW_KEY_SPACE:
			...
			break;
		case GLFW_KEY_LEFT:
			rndr->GetScene()->change_direction(1);
			//rndr->RotateArmLinkByKey(GLFW_KEY_LEFT);
			break;
		case GLFW_KEY_RIGHT:
			rndr->GetScene()->change_direction(2);
			//rndr->RotateArmLinkByKey(GLFW_KEY_RIGHT);
			break;
		case GLFW_KEY_UP:
			rndr->GetScene()->change_direction(3);
			//rndr->RotateArmLinkByKey(GLFW_KEY_UP);
			break;
		case GLFW_KEY_DOWN:
			rndr->GetScene()->change_direction(4);
			//rndr->RotateArmLinkByKey(GLFW_KEY_DOWN);
			break;
		case GLFW_KEY_X:
			rndr->GetScene()->change_direction(5);
			break;
		case GLFW_KEY_0:
			//std::cout << rndr->GetScene()->kd_trees[rndr->GetScene()->selected_data_index] << std::endl;
			break;
